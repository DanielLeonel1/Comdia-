<?php 
  ...
  $indice=0;
    $Desayuno=NULL;
    $conteo="SELECT count(*) as n FROM (((((((cena inner join carnes on cena.idCarne=carnes.ID_CARNES) inner join frutas on cena.idFruta=frutas.ID_FRUTAS) inner join verduras on cena.idVerdura=verduras.ID_VERDURAS)  inner join cereales on cereales.ID_CEREALES=cena.idCereal) inner join leguminosas on cena.idLeguminosa=leguminosas.ID_LEGUMINOSAS)inner join grasasinp on grasasinp.ID_GRASASINP=cena.idGrasaSinProteinas)inner join grasaconp on grasaconp.ID_GRASACONP=cena.idGrasaConProteinas) inner join leche on leche.ID_LECHE=cena.idLeche where idIniciar_sesion='$idUser'";
    $cont=$conne->query($conteo);
    $row2 = $cont->fetch();
    $total= $row2["n"];//
    $sql="SELECT cena.ID_Dieta, cena.fechaHora, cena.TOTALcalorias,cena.TOTALcarbohidratos, cena.TOTALproteinas, cena.TOTALfibras, cena.TOTALgrasas, cena.TOTALcolesterol, carnes.NombreCar as carne,  frutas.NombreFru as fruta, verduras.NombreVerd as verdura, cereales.NombreCer as cereal, leguminosas.NombreLegu as leguminosas, grasaconp.NombreGrac as grasaconp, grasasinp.NombreGras as grasasinp, leche.NombreLec as leche FROM (((((((cena inner join carnes on cena.idCarne=carnes.ID_CARNES) inner join frutas on cena.idFruta=frutas.ID_FRUTAS) inner join verduras on cena.idVerdura=verduras.ID_VERDURAS)  inner join cereales on cereales.ID_CEREALES=cena.idCereal) inner join leguminosas on cena.idLeguminosa=leguminosas.ID_LEGUMINOSAS)inner join grasasinp on grasasinp.ID_GRASASINP=cena.idGrasaSinProteinas)inner join grasaconp on grasaconp.ID_GRASACONP=cena.idGrasaConProteinas) inner join leche on leche.ID_LECHE=cena.idLeche where idIniciar_sesion='$idUser'";
   // echo $sql;
