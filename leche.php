...
include("conexion.php");
  $indice=0;
    $alumno=NULL;
    $conteo="SELECT count(*) as n FROM leche";
    $cont=$conne->query($conteo);
    $row2 = $cont->fetch();
    $total= $row2["n"];//
    $sql="SELECT  * FROM leche WHERE category='1'"; 
    $consulta=$conne->query($sql);
   // echo $sql; 
    while ($row = $consulta->fetch()){
      $verduras[$indice]              =new stdClass;
      $verduras[$indice]->Nombre          =$row["NombreLec"];
...
<table class="table table-success">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td> <img src="img/leche/<?= $cel->imge;?>" height ="100px" width = "140" ></img></td>
...
<?php 
  $indice=0;
    ...
    $row2 = $cont->fetch();
    $total= $row2["n"];//
    $sql="SELECT  * FROM leche WHERE category='2'"; 
    $consulta=$conne->query($sql);
   // echo $sql; 
    while ($row = $consulta->fetch()){
      $verduras[$indice]              =new stdClass;
...
<table class="table table-warning">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><?=$cel->Colesterol;?> </td>
      <td><?=$cel->DiabetesTipo ;?></td>
      <td> <img src="img/leche/<?= $cel->imge;?>" height ="100px" width = "130" ></img></td>
...
<?php 
  $indice=0;
    ...
    $row2 = $cont->fetch();
    $total= $row2["n"];//
    $sql="SELECT  * FROM leche WHERE category='3'"; 
    $consulta=$conne->query($sql);
   // echo $sql; 
    while ($row = $consulta->fetch()){
      $verduras[$indice]              =new stdClass;
...
<table class="table table-danger">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><?=$cel->Colesterol;?> </td>
      <td><?=$cel->DiabetesTipo ;?></td>
      <td> <img src="img/leche/<?= $cel->imge;?>" height ="80px" width = "100"></img></td>
