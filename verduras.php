...
include("conexion.php");
  $indice=0;
    $alumno=NULL;
    $conteo="SELECT count(*) as n FROM verduras";
    $cont=$conne->query($conteo);
    $row2 = $cont->fetch();
    $total= $row2["n"];//
    $sql="SELECT  * FROM verduras WHERE category='1'"; 
    $consulta=$conne->query($sql);
   // echo $sql; 
    while ($row = $consulta->fetch()){
      $verduras[$indice]              =new stdClass;
      $verduras[$indice]->Nombre          =$row["NombreVerd"];
...
<table class="table table-success">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td> <img src="img/verduras/<?= $cel->imge;?>" height ="100px" width = "140" ></img></td>
...
<?php 
  $indice=0;
    ...
    $row2 = $cont->fetch();
    $total= $row2["n"];//
    $sql="SELECT  * FROM verduras WHERE category='2'"; 
    $consulta=$conne->query($sql);
   // echo $sql; 
    while ($row = $consulta->fetch()){
      $verduras[$indice]              =new stdClass;
...
<table class="table table-warning">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><?=$cel->Colesterol;?> </td>
      <td><?=$cel->DiabetesTipo ;?></td>
      <td> <img src="img/verduras/<?= $cel->imge;?>" height ="120px" width = "140" ></img></td>
...
<?php 
  $indice=0;
    ...
    $row2 = $cont->fetch();
    $total= $row2["n"];//
    $sql="SELECT  * FROM verduras WHERE category='3'"; 
    $consulta=$conne->query($sql);
   // echo $sql; 
    while ($row = $consulta->fetch()){
      $verduras[$indice]              =new stdClass;
