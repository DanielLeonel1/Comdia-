...
include("conexion.php");
  $indice=0;
    $alumno=NULL;
    $conteo="SELECT count(*) as n FROM cereales";
    $cont=$conne->query($conteo);
    $row2 = $cont->fetch();
    $total= $row2["n"];//
    $sql="SELECT  * FROM cereales WHERE category='1'"; 
    $consulta=$conne->query($sql);
   // echo $sql; 
    while ($row = $consulta->fetch()){
      $verduras[$indice]              =new stdClass;
      $verduras[$indice]->Nombre          =$row["NombreCer"];
...
<table class="table table-success">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td> <img src="img/cereales/<?= $cel->imge;?>" height ="100px" width = "110px"></img></td>
...
<?php 
  $indice=0;
    ...
    $row2 = $cont->fetch();
    $total= $row2["n"];//
    $sql="SELECT  * FROM cereales WHERE category='2'"; 
    $consulta=$conne->query($sql);
   // echo $sql; 
    while ($row = $consulta->fetch()){
      $verduras[$indice]              =new stdClass;
...
<table class="table table-warning">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><?=$cel->Colesterol;?> </td>
      <td><?=$cel->DiabetesTipo ;?></td>
      <td> <img src="img/cereales/<?= $cel->imge;?>" height ="95px" width = "95px" ></img></td>
...
<?php 
  $indice=0;
    ...
    $row2 = $cont->fetch();
    $total= $row2["n"];//
    $sql="SELECT  * FROM cereales WHERE category='3'"; 
    $consulta=$conne->query($sql);
   // echo $sql; 
    while ($row = $consulta->fetch()){
      $verduras[$indice]              =new stdClass;
